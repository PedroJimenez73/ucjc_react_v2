            ...
            <>
            {
                ...
                ?  
                    <div className="evaluation-instructions">
                        <p><b>Instrucciones evaluación</b></p>
                        <p>A continuación podrá llevar a cabo la evaluación de este curso. Lea atentamente cada una de las preguntas y sus respuestas y seleccione en cada una de ellas las respuestas correctas.</p> 
                        <p>Es importante que una vez que comience esta evaluación conteste a las  preguntas que la componen antes de salir, para que se registre correctamente su puntuación.</p>
                        <ul className="regular">
                            <li>Cada pregunta puede tener una respuesta correcta o varias, indicándolo en el enunciado en el segundo caso.</li>
                            <li>Para superar esta prueba deberá obtener un <b>50% de aciertos.</b></li>
                            <li>Dispone de {cmiDataState.evaluationData.attemps} intentos</li>
                            <li>Una vez realizado cada intento, podrá comprobar los resultados obtenidos.</li>
                        </ul>
                        <div className="flex j-end a-center m-t">
                            <button className="outline" onClick={() => this.handleStartEvaluation()}>Comenzar Evaluación</button>
                        </div>
                    </div>
                : '' 
            }
            {
                ...
                ?  
                    <div className="evaluation-instructions">
                        ...
                        <p>Recuerde leer atentamente cada una de las preguntas y sus respuestas y seleccione en cada una de ellas las respuestas correctas.</p> 
                        <p>Es importante que una vez que comience esta evaluación conteste a las  preguntas que la componen antes de salir, para que se registre correctamente su puntuación.</p>
                        <ul className="regular">
                            <li>Cada pregunta puede tener una respuesta correcta o varias, indicándolo en el enunciado en el segundo caso.</li>
                            <li>Para superar esta prueba deberá obtener un <b>50% de aciertos.</b></li>
                            {
                                ...
                                ?
                                <li>Dispone de 1 intento mas.</li>
                                :
                                <li>Dispone de {cmiDataState.evaluationData.attemps} intentos mas.</li>
                            }
                            <li>Una vez realizado cada intento, podrá comprobar los resultados obtenidos.</li>
                        </ul>
                        <div className="flex j-end a-center m-t">
                ...
                <div className="final-evaluation">
                    {EvaluationInfo.questions.map((question, index) => {
                        ...
                        return <div key={index}>
                            <p>{index + 1 + '.- '}{question.ennunciate}</p>
                            ...
                            {this.state.showAnswers && question.feedback !== '' ? <p className="feedback">{question.feedback}</p> : ''}
                        </div>
                    })}
                    <div className="flex j-end a-center m-t">
                        ...
                        {!this.state.showAnswers ? 
                                <button disabled={!this.state.validForm} 
                                        ...
                                        onClick={() => {setResults()}}>Enviar</button> : null}
                    </div>
                    {
                        this.state.showAnswers
                        ? 
                        <div>
                            {
                                cmiDataState.evaluationData.corrects / this.state.userAnswers.length >= 0.5
                                ?
                                <p className="results">Enhorabuena, ha superado la evaluación. La puntuación obtenida es {cmiDataState.evaluationData.corrects + '/' + this.state.userAnswers.length}</p>
                                : 
                                <p className="results">Lo sentimos, no ha superado la evaluación. La puntuación obtenida es {cmiDataState.evaluationData.corrects + '/' + this.state.userAnswers.length}</p>
                            }
                            {
                                ...
                                <div className="flex column j-center a-center">
                                    {
                                        cmiDataState.evaluationData.attemps === 1
                                        ?
                                        <p className="results">Dispone de 1 intento más. Le recomendamos repasar detenidamente la formación antes de realizarlo.</p>
                                        :
                                        <p className="results">Dispone de {cmiDataState.evaluationData.attemps} intentos más. Le recomendamos repasar detenidamente la formación antes de realizarlo.</p>
                                    }
                                    <button className="outline" onClick={() => {navToPage(2)}}>Repasar</button>
                                </div>
                                :
                                <div>
                                    {
